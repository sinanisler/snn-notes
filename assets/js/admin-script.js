    ...
    const SnnNotes = {
        ...
        renderNotes: function () {
            ...
            allNotes.forEach(note => {
                ...
                // Drop zone for tags
                $noteItem.on('dragover', function (e) {
                    e.preventDefault();
                    $(this).addClass('drag-over');
                });

                $noteItem.on('dragleave', function () {
                    $(this).removeClass('drag-over');
                });

                $noteItem.on('drop', (e) => {
                    e.preventDefault();
                    $noteItem.removeClass('drag-over');

                    const tagId = e.originalEvent.dataTransfer.getData('tagId');
                    if (tagId) {
                        this.addTagToSpecificNote(note.id, parseInt(tagId));
                    }
                });
        ...
        addTagToSpecificNote: function (noteId, tagId) {
            const self = this;
            const note = allNotes.find(n => n.id === noteId);

            if (!note) return;

            const currentTags = note.tags.map(t => t.id);

            if (currentTags.includes(tagId)) {
                self.showStatus('Tag already assigned', 'info');
                return;
            }
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        self.showStatus('Tag added to note', 'success');
                        self.loadNotes(); // Refresh list to show new tag pill

                        // If we modified the currently open note, refresh it too
